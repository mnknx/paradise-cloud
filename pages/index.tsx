...
const IMGS = {
  'Paradiseum': '/images/wallet_paradiseum.png',
...
function Home({ className }: { className?: string }) {
  ...
  const _onClickCrust = useCallback(async () => {
    try {
      ...
      if (!user.crust.provider) {
        setError(`Paradiseum Wallet not installed`)
        return
      }
  ...
  const _onClickNear = useCallback(async () => {
    ...
    await user.near.wallet.requestSignIn(nearConfig.contractName, 'Paradiseum Cloud');
  }, [user, t]);
  ...
  const wallets = useMemo<Wallet[]>(() => {
    return [
      {
        group: 'Paradiseum',
        name: 'Paradiseum Wallet',
        image: '/images/wallet_paradiseum.png',
  ...
  return (
    ...
    <div className={className}>
      ...
      <div className="center_panel">
        ...
        <Pixel className="pixel_right" width={'8.57rem'} position="right" fullH={true} color="#1159d7" fillColor="#995ff7" />
      </div>
